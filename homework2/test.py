...
class Test(unittest.TestCase):
    ...
    @patch('sys.argv', ['main.py', 'express'])  # Подменяем аргументы командной строки
    # Захватываем вывод
    def test_1(self, mock_output):
        main()  # Вызываем main
        self.assertEqual(mock_output.getvalue(), '''@startuml
"express" --> "qs"
"express" --> "depd"
"express" --> "etag"
"express" --> "send"
...
''')
    ...
    @patch('sys.stdout', new_callable=StringIO)
    @patch('sys.argv', ['main.py', 'norgshdtydjnlufnbtbilt8 '])  # Подменяем аргументы командной строки
    # Захватываем вывод
    def test_2(self, mock_output):
        main()  # Вызываем main
        self.assertEqual(mock_output.getvalue(), '''Failed to fetch package data: 404
@startuml
@enduml
